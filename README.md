...
There are base classes - `FragmentCore` or `BindableFragmentCore` that should be extended. As with Activity version `BindableFragmentCore` expets to type-params: `* Binding` class generated by databinding library and the type of ViewModel corresponding to this Fragment.
