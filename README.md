...
1. Extend `ActivityCore` or `BindableActivityCore` for databinding capabilities. 

_Note, that the first type-parameter of `BindableActivityCore` generic is `ViewModel` and the second - binding class `ActivityMainBinding` autogenerated by android databinding engine for your `MainActivity`._ 
