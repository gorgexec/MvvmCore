...
### Activity
Extend `ActivityCore` or `BindableActivityCore` (for databinding capabilities). Note, that the first type parameter in `Bindable*` case is autogenerated by android databinding engine binding class for the activity. Then call MvvmCore `bind()` method right from the start of the `onCreate` callback.
