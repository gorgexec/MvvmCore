...
### Activity
Extend `ActivityCore` or `BindableActivityCore` (for databinding capabilities). Note, that the first type-parameter of `BindableActivityCore` generic is binding class for the activity autogenerated by android databinding engine (`ActivityMainBinding`). Then call MvvmCore `bind()` method right from the start of the `onCreate` callback.
